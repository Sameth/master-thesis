...
//        if (!minimallyconnected(next)) return;
        ...
        For(i, n) For(j, i) {
            preset [i] = 0;
            preset [j] = 0;
            for (int e : edges [i]) preset [e] = 0;
            for (int e : edges [j]) preset [e] = 0;

            ll emptypp = count_pp_lim(edges, preset);

            preset [i] = -1;
            preset [j] = -1;
            for (int e : edges [i]) preset [e] = -1;
            for (int e : edges [j]) preset [e] = -1;
            
            najhorsiodhad = max(najhorsiodhad, 2*mypp + emptypp);
        }
