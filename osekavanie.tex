...
\begin{defn}
    Rozhodovací problém \emph{zoznamového $L(2,1)$-farbenia} definujeme nasledovne: Inštanciou je
    graf $G$, prirodzené číslo $k$ a zobrazenie $c: V(G) \to 2^{\{0 \ldots k\}}$. Inštancia je
    riešiteľná práve vtedy, keď existuje $L(2,1)$ farbenie $f$ grafu $G$ s rozsahom $k$, pre ktoré platí
    $\forall v \in V(G): f(v) \in c(v)$.

    Zobrazenie $c$ budeme nazývať \emph{zoznam povolených hodnôt}.
\end{defn}

Ľubovoľné riešenie problému zoznamového $L(2,1)$-farbenia vieme prerobiť na riešenie
problému $L(2,1)$-farbenia -- pre každý vrchol $v$ určíme $c(v) = \{0 \ldots k\}$. Taktiež sa
však ukazuje, že mnohé riešenia problému $L(2,1)$-farbenia sa dajú prerobiť na riešenia problému
zoznamového $L(2,1)$-farbenia. Ukážeme si to na príklade algoritmu Junosza-Szaniawski.

Pripomenieme si, že algoritmus Junosza-Szaniawski počítal postupne tabuľky $T_0, T_1, \ldots T_{2n}$.
Tabuľka $T_i$ obsahuje reprezentáciu všetkých čiastočných $i$-$L(2,1)$-farbení grafu. Pre každé
...
všetky čiastočné farbenia, ktoré priradili farbu $i$ vrcholu, ktorý ju podľa zoznamu povolených
hodnôt nemôže dostať. Kvôli obmedzeniu zoznamom povolených hodnôt nemusí platiť, že 
